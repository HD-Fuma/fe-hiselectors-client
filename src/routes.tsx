...
export function routeMatchesPath(route: AppRoute, path: string): boolean {
  return route.path === path
    || (route.id === 'campaign-detail' && /^\/campaigns\/[^/]+$/.test(path))
    || (route.id === 'public-shop' && /^\/shop\/(?!groups(?:\/|$)|profile(?:\/|$))[^/]+$/.test(path))
    || (route.id === 'shop-product-detail' && /^\/product\/[^/?]+/.test(path))
    || (route.id === 'owner-shop-group' && /^\/shop\/(?!groups(?:\/|$)|profile(?:\/|$))[^/]+\/(?!products(?:\/|$))[^/]+$/.test(path))
    || (route.id === 'group-edit' && /^\/shop\/groups\/[^/]+\/edit$/.test(path))
    || (route.id === 'group-campaign-create' && /^\/shop\/groups\/new\/campaign\/[^/]+$/.test(path))
}

export function selectRouteByPath(path: string): AppRoute {
  return routes.find((route) => routeMatchesPath(route, path)) ?? routes[0]
}
...
export function getRouteRedirect(route: AppRoute, session: AuthSession | null): AppPath | null {
